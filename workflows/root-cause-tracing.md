...
- Use appropriate debugging tools for your environment
- Check inputs, outputs, and state changes
...
```
✅ "The data loading isn't working. Let me trace the root cause:
    1. Reproduce: Data not appearing after initialization
    2. Observe: Check the actual configuration and data flow
    3. Hypothesize: Maybe wrong configuration? Maybe state not refreshing?
    4. Test: Found configuration mismatch in initialization
    5. Fix: Use correct configuration and ensure proper state refresh
    6. Verify: Tests now pass
    7. Document: Added correct configuration to test helpers"
```
...
- NEVER skip tests to avoid fixing complicated issues
- ALWAYS trace the root cause of test failures before implementing fixes
- When implementation changes break tests, update test assertions to match new behavior
...
### Multiple matches / Ambiguous selection
- Read the error message for details
- Make identifiers more specific
- Use explicit selection only if multiple matches are expected

### Not found / Missing element
- Check actual state vs expected state
- Verify the feature is implemented
- Fix application bugs before modifying tests

### Timeout / Hanging
- Check if expected condition is achievable
...
1. **Write a failing test FIRST** (integration or end-to-end test preferred)
2. **Test through the full stack** - NOT individual functions in isolation
3. **Use descriptive test name** including bug reference (e.g., `"BUG-123: Expected behavior not working"`)
